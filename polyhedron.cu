...
*/
__device__ char InPolyhedron( int F, tPointd q, tPointd bmin, tPointd bmax, int radius )
{
...
__device__ int ComputeBox( int F, tPointd bmin, tPointd bmax )
{
  int i, j;
  double radius;
...
__global__ void RandomRay( tPointd ray, int radius )
{
  double x, y, z, w, t;
...
__global__ void AddVec( tPointd q, tPointd ray )
{
  int i;
...
---------------------------------------------------------------------*/
__device__ char	SegPlaneInt( tPointi T, tPointd q, tPointd r, tPointd p, int *m)
{
    tPointd N; double D;
    tPointd rq;
    double num, denom, t;
...
---------------------------------------------------------------------*/
__device__ int  PlaneCoeff( tPointi T, tPointd N, double *D )
{
    int i;
...
a - b ==> c.
---------------------------------------------------------------------*/
__global__ void    SubVec( tPointd a, tPointd b, tPointd c )
{
   int i;
...
---------------------------------------------------------------------*/
__global__ void	NormalVec( tPointd a, tPointd b, tPointd c, tPointd N )
{
    N[X] = ( c[Z] - a[Z] ) * ( b[Y] - a[Y] ) -
           ( b[Z] - a[Z] ) * ( c[Y] - a[Y] );
    N[Y] = ( b[Z] - a[Z] ) * ( c[X] - a[X] ) -
...
__device__ char InTri3D( tPointi T, int m, tPointd p )
{
...
__device__ char InTri2D( tPointd Tp[3], tPointd pp )
{
...
__device__ char  SegTriInt( tPointi T, tPointd q, tPointd r, tPointd p )
{
    int code = '?';
    int m = -1;
...
__device__ char SegTriCross( tPointi T, tPointd q, tPointd r )
{
   int vol0, vol1, vol2;
...
*/
__device__ char BoxTest ( int n, tPointd a, tPointd b )
{
